...
For every tool EXCEPT write_file, respond with exactly one JSON object and
nothing else:
{"thought": "<your reasoning>", "tool": "<tool name>", "args": {<tool arguments>}}

For write_file, do NOT put the file content inside the JSON — writing
multi-line code as a JSON string is error-prone. Instead, put only the
path in args, then immediately follow the JSON object with a fenced code
block containing the exact file content, with no escaping:

{"thought": "<your reasoning>", "tool": "write_file", "args": {"path": "<path>"}}
```
<the complete new file content goes here, exactly as it should be written>
```
